...
// 용도 : n초 후에 m함수를 실행하는 클래스
template<typename EventTarget>
class GameEngineTimeEventer
{
public:
	class TimeEvent
	{
	...
	public:
		TimeEvent() : m_Time(0.0f), m_MaxTime(0.0f), m_Object(), m_StartFunc(), m_EndFunc(), m_StayFunc(), m_StopFunc()
		{
	...
	public:
		bool Update()
		{
			...
			{
				// 시간을 계산해준다.
				m_Time -= GameEngineTime::GetInst().GetDeltaTime();
...
public: // constructer destructer
	GameEngineTimeEventer() : m_CurEvent(0), m_Loop(false), m_Events()
	{

	}

	~GameEngineTimeEventer()
	{
